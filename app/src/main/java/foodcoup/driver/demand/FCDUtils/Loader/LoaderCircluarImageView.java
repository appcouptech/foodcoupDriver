...
public class LoaderCircluarImageView  extends AppCompatImageView implements LoaderView {

    private LoaderController loaderController;
    private int defaultColorResource;

    public LoaderCircluarImageView(Context context) {
        super(context);
        init(null);
    }

    public LoaderCircluarImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public LoaderCircluarImageView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(AttributeSet attrs) {
        ...
        @SuppressLint("CustomViewStyleable") TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.loader_view, 0, 0);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        loaderController.onDrawCirclar(canvas);
